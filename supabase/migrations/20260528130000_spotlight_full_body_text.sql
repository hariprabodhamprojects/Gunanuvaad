-- Spotlight was truncating the slide body to 480 chars (swadhyay) / 520 (notes) / 320 (smruti).
-- With the new in-card scroll fade, users can read long bodies — return the full stored text.
...
begin
  ...
  -- Assemble: interleave note/smruti/swadhyay; skip _2 slot when it duplicates _1.
  ...
  if v_sw2   is not null and v_sw2_id   is distinct from v_sw1_id   then slides := slides || jsonb_build_array(v_sw2);   end if;
