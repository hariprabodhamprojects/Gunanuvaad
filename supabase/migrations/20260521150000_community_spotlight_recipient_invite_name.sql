-- Community spotlight ghuns: show recipient invite name and logo placeholder when not signed up.
...
begin
  ...
  select
    jsonb_build_object(
      ...
      'recipient_avatar_url', coalesce(nullif(trim(pr.avatar_url), ''), '/logo.png')
    ),
    dn.id
