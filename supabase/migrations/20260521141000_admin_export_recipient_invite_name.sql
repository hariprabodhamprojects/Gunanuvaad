-- Admin ghun export: match recipients by invite email when profile row is missing.
...
begin
  ...
  select
    ...
    coalesce(nullif(trim(pr.email), ''), nullif(trim(dn.recipient_email), ''), '') as recipient_email,
    dn.body
  ...
  left join public.allowed_emails ae
    on ae.email = lower(trim(coalesce(nullif(trim(dn.recipient_email), ''), pr.email, '')))
  where (
      coalesce(pr.display_name, ae.display_name, pr.email, dn.recipient_email, '') ilike ('%' || q || '%')
    )
