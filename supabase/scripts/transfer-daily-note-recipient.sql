...
--
-- Example: ghun written for milanrupapara89@gmail.com should belong to milanrocks07@gmail.com
...
where lower(trim(email)) = lower(trim('milanrocks07@gmail.com'));

-- ── 2) Transfer (uncomment after preview looks right) ─────────────────────────
/*
update public.daily_notes dn
set
  recipient_email = lower(trim('milanrocks07@gmail.com')),
  recipient_id = (
    select p.id
    from public.profiles p
    where lower(trim(p.email)) = lower(trim('milanrocks07@gmail.com'))
    limit 1
  )
where lower(trim(dn.recipient_email)) = lower(trim('milanrupapara89@gmail.com'));
*/
