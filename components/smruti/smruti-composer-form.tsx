...
type SmrutiComposerFormProps = {
  className?: string;
};

export function SmrutiComposerForm({ className }: SmrutiComposerFormProps) {
  ...
  return (
    <form
      action={submit}
      className={cn(
        "relative mx-auto flex w-full max-w-lg min-h-0 flex-1 flex-col",
        className,
      )}
    >
      <input
      ...
      {/* Scrollable body — photo, thumbs, caption; desktop is normal document flow */}
      <div
        ref={scrollRef}
        className={cn(
          "flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto overscroll-y-contain sm:gap-4",
          "pb-2 lg:flex-none lg:overflow-visible lg:pb-0",
        )}
      >
        ...
        {!n ? (
          ...
          >
            ...
            <img
              ...
              src={previews[idx]}
              alt=""
              className="relative z-[2] mx-auto block max-h-[min(40svh,18rem)] w-full max-w-full object-contain object-center sm:max-h-[min(44svh,22rem)]"
            />
            {n > 1 ? (
        ...
        <div className="pt-1">
          ...
          <Textarea
            ...
            className={cn(
              "min-h-[6.5rem] resize-y rounded-xl border border-border/60 bg-card px-3 py-3 sm:min-h-[7.5rem] sm:px-3.5",
              "font-heading text-base font-medium leading-relaxed text-primary",
        ...
        {/* Desktop — actions under the caption in normal flow */}
      ...
      {/* Mobile — docked footer (flex sibling, not fixed — avoids scroll-container / transform bugs) */}
      <div
        className={cn(
          "shrink-0 border-t border-border/60 bg-background/95 backdrop-blur-md lg:hidden",
          "-mx-3 px-3 py-2.5 sm:-mx-4 sm:px-4",
        )}
      >
