...
export function SwadhyayComments({
  ...
  // Live-update comments, reactions, and pin state for this topic.
  // Reactions don't carry topic_id, so that listener is unfiltered — volume
  // is low in practice and the server fetch decides what's relevant.
  useRealtimeRefresh({
    channel: `swadhyay-topic-${topic.id}`,
    subscriptions: [
      { table: "swadhyay_comments", filter: `topic_id=eq.${topic.id}` },
